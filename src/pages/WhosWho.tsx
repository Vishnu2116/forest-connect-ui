              ...
              {elementLeadership.map(o => (
                <article key={o.name} className="bg-card border border-border rounded-xl p-6 text-center hover:shadow-card transition group">
                  ...
                  <div className="mt-3 pt-3 border-t border-border space-y-1.5 text-xs text-muted-foreground">
                    {o.phone && <div className="flex items-center justify-center gap-1.5"><Phone className="h-3 w-3" /> {o.phone}</div>}
                    {o.mobile && <div className="flex items-center justify-center gap-1.5"><Smartphone className="h-3 w-3" /> {o.mobile}</div>}
                    {o.email && <div className="flex items-center justify-center gap-1.5"><Mail className="h-3 w-3" /> {o.email}</div>}
                  </div>
                </article>
              ))}
            </div>
